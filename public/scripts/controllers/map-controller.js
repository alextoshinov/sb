"use strict";
angular.module("showyBulgariaApp").controller("MapController",
...
*/
	...
	var updateViewportToDefault = function() {
		if (google.loader.ClientLocation) {
			...
			// Default to a central view of the Bulgaria
			center = new google.maps.LatLng(42.505, 25.525); // Center over the Americas since they have the most hikes currently.
			zoomLevel = 8;
		}
	};
	...
	*/
	...
	var initStaticData = function() {
		$http({method: "GET", url: "/api/v1/hikes", params: { fields: "distance,location,name,string_id" }, cache:resourceCache}).
                //$http({method: "GET", url: "/data/maps.json", params: { fields: "distance,location,name,string_id" }, cache:resourceCache}).        
			success(function(data, status, headers, config) {
				mergeMarkers(data);
